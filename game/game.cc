...
void Game::_InitGLFW()
{
	...
	{
		DEBUG_LOG("Game", LOG_FATAL, "GLFW initialization failed!");
		glfwTerminate();
	}
}
...
void Game::_InitWindow(const char* title, bool resizeable)
{
	...
	if (_window == nullptr)
	{
		DEBUG_LOG("Game", LOG_FATAL, "GLFW window initialization failed!");
		glfwTerminate();
	}
...
void Game::_InitGLEW()
{
	...
	if (glewInit() != GLEW_OK)
	{
		DEBUG_LOG("Game", LOG_FATAL, "GLEW initialization failed!");
		glfwTerminate();
	}
}
...
void Game::_InitShaders()
{
	...
	_shaders.push_back(new Shader(_GL_VERSION_MAJOR, _GL_VERSION_MINOR, "shaders/shadow.vert", "shaders/shadow.frag"));
...
{
	_textures.push_back(new Texture("resources/textures/models/matoshi.png", GL_TEXTURE_2D));
	_textures.push_back(new Texture("resources/textures/models/matoshi_emissive.png", GL_TEXTURE_2D));
}
...
void Game::_InitModels()
{
	...
	std::vector<Vertex> torus = LoadOBJ("resources/models/matoshi.obj");
...
/// Create components, then entities, then all the different variants of systems in the ECS.
void Game::_InitECS()
{
	...
	// (2) Create entities
	_entity = _ecs.MakeEntity(transformComponent, movementControl, cameraComponent);
...
void Game::_UpdateInput(GLFWwindow* window, Texture* tex)
{
	...
	{
		tex->SetTexture("resources/edit32.png", true);
	}
}
